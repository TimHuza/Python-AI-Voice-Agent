...
async def handle_text_messages(decoded, twilio_ws, sts_ws, streamsid):
    ...
    if decode["type"] == "FunctionCallRequest":
...
async def sts_receiver(sts_ws, twilio_ws, streams_id_queue):
    print("sts_receiver started")
    streamsid = await streams_id_queue.get()

    async for message in sts_ws:
        if type(message) == str:
            decoded = json.loads(message)
            await handle_text_messages(decoded, twilio_ws, sts_ws, streamsid)
            continue
        ...
        media_message = {
            "event": "media",
            "streamSid": streamsid,
            "media": {"payload": base64.b64encode(raw_mluaw).decode("ascii")}
        }
